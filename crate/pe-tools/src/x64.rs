...
use ntapi::{ntldr::LDR_DATA_TABLE_ENTRY, ntpebteb::PEB, winapi_local::um::winnt::__readgsqword};
use winapi::{
    ctypes::c_void,
    shared::{
        basetsd::DWORD64,
    ...
    um::{
        ...
        winnt::{
            ...
            IMAGE_NT_HEADERS64, IMAGE_ORDINAL64, IMAGE_SNAP_BY_ORDINAL64, IMAGE_THUNK_DATA64,
    ...
    pub fn resolve_import(
        ...
        unsafe {
            ...
            while (*import_discriptor).Name != 0x0 {
                ...
                let mut thunk = (target_image_base_address as u64
                    + (*import_discriptor).FirstThunk as u64)
                    as *mut IMAGE_THUNK_DATA64;

                while (*thunk).u1.AddressOfData() != &0x0 {
                    if orig_thunk != null_mut()
                        && IMAGE_SNAP_BY_ORDINAL64(*(*orig_thunk).u1.Ordinal())
                    {
                        ...
                        *(*thunk).u1.Function_mut() = p_get_proc_adress(lib, fn_ordinal) as _;
                        ...
                        let fn_name = (target_image_base_address as u64
