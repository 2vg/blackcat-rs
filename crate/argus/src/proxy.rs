use std::{net::{Shutdown, SocketAddr}, path::{Path, PathBuf}, sync::Arc};

use anyhow::*;
use async_h1::{client as h1_client, server as h1_server};
use async_std::io::{copy, Cursor, prelude::*};
use async_std::prelude::*;
use async_std::net::{TcpListener, TcpStream};
use async_std::task;
use async_native_tls::{TlsStream, TlsAcceptor, TlsConnector};
...
async fn handle_conn(server: Arc<Server>, client_stream: TcpStream) -> Result<()> {
    ...
    match req.method() {
        Method::Connect => {
            let  (client_stream, target_stream) = handle_handshake(&server.container, req.url(), client_stream).await?;
...
async fn handle_handshake(
    container: &cert_util::CAContainer, url: &Url, mut client_stream: TcpStream
)-> Result<(
...
pub async fn modify_request(mut input: Request) -> Result<Request> {
	input.insert_header("User-Agent", "Homozilla/5.0 (Checker/1.14.514; homOSeX 8.10)");
	Ok(input)
}
