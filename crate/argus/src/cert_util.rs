...
pub fn save_key(file: &Path, key: &PKeyRef<Private>, pass_phrase: &str) -> Result<()> {
    ...
    if pass_phrase.len() == 0 {
        file.write_all(
            &key.rsa().unwrap().private_key_to_pem_passphrase(
...
pub fn native_identity(certificate: &X509, key: &PKey<Private>, password: &str) -> Result<Vec<u8>> {
    Ok(Pkcs12::builder()
        .build(password, "", key, certificate)?
        .to_der()?)
}

pub fn spoof_certificate(certificate: &X509, ca: &CAContainer) -> Result<X509> {
...
pub fn get_binary_from_file(path: &Path) -> Result<Vec<u8>> {
    let mut f =
        File::open(path).with_context(|| format!("could not opening the file: {:?}", path))?;
